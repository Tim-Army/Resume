...
# Prepare a new linked branch or resume an existing one. A previous run may
# have created the branch before stopping, so branch existence is not an error.
...
if [[ "$LOCAL_BRANCH_EXISTS" == true ]]; then
  ...
  log "Checking out existing remote branch: origin/$BRANCH_NAME"
...
if git diff --cached --quiet; then
  log "No new manuscript changes were detected; continuing the existing branch workflow."
else
...
log "Pushing or synchronizing branch with origin."
git push --set-upstream origin HEAD

# Determine whether the branch contains work that is not already in the base
# branch. This prevents an empty pull-request attempt on a fully completed rerun.
git fetch origin "$BASE_BRANCH" >/dev/null 2>&1 || true
COMMITS_AHEAD="$(git rev-list --count "origin/${BASE_BRANCH}..HEAD" 2>/dev/null || printf '0')"
...
EOF_PR
    ...
    warn "Branch has no commits ahead of '$BASE_BRANCH'; no pull request is required."
  fi
fi
